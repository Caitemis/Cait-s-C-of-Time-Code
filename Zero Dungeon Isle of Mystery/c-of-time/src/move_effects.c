...
//Implements Soak, thanks to Sudzoof for writing this! I now have too much power.
...
//Vacuum Cut variant, deals damage equal to Level times 4.
...
//Essentially Explosion, but doesn't destroy items or walls and can get EXP from defeated enemies.
...
//Signal Beam variant. Always confuses.
static bool MoveSignalBeamVariant(struct entity* user, struct entity* target, struct move* move) {
    if (!DealDamage(user, target, move, 0x100, ITEM_NOTHING)) {
        return false;
    }
...
//Psychic variant. Chance of Special Defense drop. Goes in the Frenzy Plant slot to force recharge.
...
//Psybeam variant. Always confuses, half recoil to self.
...
//Heal Block variant. Deals damage and inflicts Heal Block.
static bool MoveHealBlockVariant(struct entity* user, struct entity* target, struct move* move) {
    if (!DealDamage(user, target, move, 0x100, ITEM_NOTHING)) {
        return false;
    }
    TryInflictHealBlockStatus(user, target, true, false);
    return true;
}
    ...
    move_effect_input* data, struct entity* user, struct entity* target, struct move* move)
{
    ...
    case MOVE_SIGNAL_BEAM:
        data->out_dealt_damage = MoveSignalBeamVariant(user, target, move);
        return true;
    case MOVE_FLASH:
